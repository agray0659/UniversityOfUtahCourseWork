...
function SimTimeStep( dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution )
{
	...
	for(let i=0;i<springs.length;i++)
	{
		...
		let springForce=directionVector.mul(stiffness*(springLength-restLength));
		let lengthChangeSpeed=(velocities[springs[i].p1].sub(velocities[springs[i].p0])).dot(directionVector);
		let dampingForce=directionVector.mul(damping*lengthChangeSpeed);
		let totalForce=springForce.add(dampingForce);

		forces[springs[i].p0].inc(totalForce);
		forces[springs[i].p1].dec(totalForce);

	}
	// [TO-DO] Update positions and velocities
	for (let j=0;j<positions.length;j++) 
	{
		var acceleration=forces[j].div(particleMass);
		velocities[j].inc(acceleration.mul(dt));
